...
function checkSpoiled(inputIds: number[], ranges: number[][]) {
inputIds.forEach((id) => {
  // console.log("Checking:", id)
  ranges.forEach((range) => {
    // console.log("Against range:", range[0], range[1])
    if (range[0]! <= id && id <= range[1]!){
      //Id is fresh
      // console.log("Fresh ID found:", id)
      fresh++
    }
  })
...
checkSpoiled(ids, mergedRanges)

console.log("Fresh IDs:", fresh)
